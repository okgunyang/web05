...
<div class="table_lst_wrap" style="max-width:800px; margin:30px auto;">
	<table class="table" id="myTable">
			...
			<tr>
				...
				<td>
					<div class="indent" style="padding-left:${qna.qindent }px">
						<c:if test="${qna.qstep==0 }">
						<a href="${path1 }/admin/qnaRead.do?qno=${qna.qno }">${qna.qtitle }</a>
						</c:if>
						<c:if test="${qna.qstep>0 }">
						<a href="${path1 }/admin/replyRead.do?qno=${qna.qno }">${qna.qtitle }</a>
						</c:if>	
					</div>
				</td>
				<td>
		...
		<c:if test="${!empty sid }">
			<tr>
				<td colspan="4">
					<a href="${path1 }/admin/qnaWriteForm.do" class="btn btn-primary" style="width:70px">질문하기</a>
				</td>
			</tr>
			</c:if>
		</tbody>
